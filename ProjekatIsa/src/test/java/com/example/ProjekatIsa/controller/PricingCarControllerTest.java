...
public class PricingCarControllerTest {
	...
	public void testChangeService() throws Exception {
		PricingCarDTO pricing = new PricingCarDTO();
		...
		String json = TestUtil.json(pricing);
		this.mockMvc.perform(post(URL_PREFIX + "/changePricing/"+1L).contentType(contentType).content(json)).andExpect(status().isOk());
	}
